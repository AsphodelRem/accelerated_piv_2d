...
__global__
void LowPassFilter_kernel(cuComplex *spectrum, int U, int V, float k)
{
...
__global__
void BandPassFilter_kernel(cuComplex *spectrum, int U, int V)
{
    unsigned int v = blockDim.x * blockIdx.x + threadIdx.x;
    unsigned int u = blockDim.y * blockIdx.y + threadIdx.y;
...
void LowPassFilter(cuComplex *spectrum, unsigned int height, unsigned int width, float k)
{
    dim3 threads_per_block(kBlockSize, kBlockSize);
    dim3 grid_size((width + kBlockSize - 1) / kBlockSize,
                   (height + kBlockSize - 1) / kBlockSize);

    LowPassFilter_kernel<<<grid_size, threads_per_block>>>(spectrum, height, width, k);
}

void BandPassFilter(cuComplex *spectrum, unsigned int height, unsigned int width)
{
    dim3 threads_per_block(kBlockSize, kBlockSize);
    dim3 grid_size((width + kBlockSize - 1) / kBlockSize,
                  (height + kBlockSize - 1) / kBlockSize);

    BandPassFilter_kernel<<<grid_size, threads_per_block>>>(spectrum, height, width);
}
