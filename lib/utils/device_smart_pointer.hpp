...
template <typename T> class SharedPtrGPU {
public:
  SharedPtrGPU()
  ...
  SharedPtrGPU(const size_t size_in_bytes)
      : owners_(new unsigned long long(0))
      , size_(size_in_bytes) {
    auto status = cudaMalloc(reinterpret_cast<void **>(&this->raw_data_ptr_),
                             this->size_);
  ...
  ~SharedPtrGPU() { this->Decrement(); }

  inline T *get() const { return this->raw_data_ptr_; }

  T *operator->() const { return this->raw_data_ptr_; }

  T &operator*() const { return *this->raw_data_ptr_; }

  inline unsigned long long size() const { return this->size_; }

  SharedPtrGPU &CopyDataToHost(void *destination) {
    auto status = cudaMemcpy(destination, this->raw_data_ptr_, this->size(),
                             cudaMemcpyDeviceToHost);

    if (status != cudaSuccess) {
      throw std::runtime_error("Unable to copy data to host. Memcpy status: " +
                               std::to_string(status));
    }
  ...
  SharedPtrGPU &UploadHostData(const void *source, size_t size_in_bytes) {
    if (size_in_bytes > this->size_) {
      throw std::runtime_error("Size of the host data must be less or equal to "
                               "size of the allocated memory on device");
    }

    auto status = cudaMemcpy(this->raw_data_ptr_, source, size_in_bytes,
                             cudaMemcpyHostToDevice);

    if (status != cudaSuccess) {
      throw std::runtime_error(
          "Unable to copy data to device. Memcpy status: " +
          std::to_string(status));
    }
